      ...
      checks.forEach(check => {
        ...
        _data.read('checks', check, (err, originalCheckData) => {
          if(!err && originalCheckData) {
            // Pass it to the check validator, and let that  continue the  or log the error(s) as needed
...
workers.performCheck = originalCheckData => {
  ...
  // Checks outcome and if not sent, processes it
  processOutcome = () => {
    if (!outcomeSent) {
...
workers.alertUserToStatusChange = newCheckData => {
  ...
  _helpers.sendTwilioSms(newCheckData.userEmail, msg, err => {
    if(!err) {
...
workers.log = (originalCheckData, checkOutcome, state, alertWarranted, timeOfCheck) => {
  ...
  _logs.append(logFileName, logString, err => debug(`Logging to file ${err ? 'failed' : 'succeeded'}`))
}
    ...
    if(!err && logs && logs.length > 0) {
      logs.forEach(logName => {
        ...
        _logs.compress(logId, newFileId, err => {
          if(!err){
            // Truncate the log
            _logs.truncate(logId, err => debug(`${err ? 'Error' : 'Success'} truncating logfile`))
          } else {
...
workers.logRotationLoop = () => setInterval(() => workers.rotateLogs(), 1000 * 60 * 60 * 24)
