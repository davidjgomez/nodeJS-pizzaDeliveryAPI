        ...
        _data.read('users', userEmail, (err, userData) => {
          if(!err && userData) {
            ...
            if(userChecks.length < _config.maxChecks) {
              ...
              callback(400, {'Error' : `The user already has the maximum number of checks (${_config.maxChecks})`})
            }
          } else {
  ...
  if(id) {
    ...
    _data.read('checks', id, (err, checkData) => {
      if(!err && checkData) {
        ...
        tokens.verifyToken(token, checkData.userEmail, tokenIsValid => {
          if(tokenIsValid) {
            // Return check data
...
// Export the handlers for checks
